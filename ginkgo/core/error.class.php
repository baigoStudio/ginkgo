...
class Error {
    ...
    static function appException($excpt) {
        ...
        if (self::$configDebug['dump']) {
            $_arr_error['err_message']  = $excpt->getMessage();
            $_arr_error['err_file']     = $excpt->getFile();
            $_arr_error['err_line']     = $excpt->getLine();
            $_arr_error['err_detail']   = '';

            if (method_exists($excpt, 'getData')) {
                $_arr_error['err_detail']   = $excpt->getData('err_detail');
            }
        }
