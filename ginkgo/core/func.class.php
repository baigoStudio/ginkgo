...
class Func {
    ...
    static function ucwords($string, $delimiter = '') {
        ...
        if (!self::isEmpty($delimiter)) {
            $_str_delimiter = $delimiter;
        }

        //print_r(version_compare(PHP_VERSION, '5.4.32', '>'));

        if (version_compare(PHP_VERSION, '5.4.32', '>')) {
            $_str_return = ucwords(strtolower($string), $_str_delimiter);
        } else {
            $_arr_string = explode($_str_delimiter, $string);
            //print_r($_arr_string);
            foreach ($_arr_string as $_key=>$_value) {
                $_str_return .= $_str_delimiter . ucfirst(strtolower($_value));
            }
            $_str_return = trim($_str_return, $_str_delimiter);
        }
