...
var obj_form=$(this);obj_form.submit(function(evt){evt.preventDefault();});var opts_default={ajax_url:'',timeout:5000,class_content:{submitting:'text-info',err:'text-danger',success:'text-success'},class_icon:{submitting:'spinner-grow spinner-grow-sm',err:'fas fa-times-circle',success:'fas fa-check-circle'},msg_text:{submitting:'Submitting ...',err:'Error'},selector:{submit_btn:'[type="submit"]',empty_input:':password',jump_link:'.bg-jump'},modal:{selector:{modal:'.bg-submit-modal',content:'.bg-content',icon:'.bg-icon',msg:'.bg-msg',close:'.bg-close',ok:'.bg-ok'},btn_text:{close:'Close',ok:'Ok'},tpl:'<div class="modal fade bg-submit-modal">'+'<div class="modal-dialog modal-dialog-centered">'+'<div class="modal-content">'+'<div class="modal-body d-flex justify-content-between align-items-end">'+'<p class="lead bg-content">'+'<span class="bg-icon"></span>'+'<span class="bg-msg"></span>'+'</p>'+'<p class="bg-jump">'+'</p>'+'</div>'+'<div class="modal-footer">'+'<button type="button" class="btn btn-outline-secondary btn-sm bg-close" data-dismiss="modal">Close</button>'+'<a href="#" class="btn btn-primary btn-sm bg-ok">Ok</a>'+'</div>'+'</div>'+'</div>'+'</div>',delay:5000},jump:{url:'',text:'',icon:'spinner-grow spinner-grow-sm',delay:3000}};var opts=$.extend(true,opts_default,options);var modal_selector=opts.modal.selector.modal;if($(modal_selector).length<1){$('body').append(opts.modal.tpl);}
...
_msg_html+='</span>';if(_rcode_status=='y'&&opts.jump.url.length>0&&opts.jump.text.length>0){var _jump_url=opts.jump.url;if(typeof opts.jump.attach_key!='undefined'&&typeof _attach_value!='undefined'){if(_jump_url.indexOf('?')){_str_conn='&';}else{_str_conn='?';}
...
break;default:obj_form.find(opts.selector.submit_btn).removeAttr('disabled');break;}},submitFunc:function(){var _str_conn;if(typeof opts.ajax_url=='undefined'||opts.ajax_url==''){opts.ajax_url=obj_form.attr('action');}
if(opts.ajax_url.indexOf('?')>0){_str_conn='&';}else{_str_conn='?';}
$.ajax({url:opts.ajax_url+_str_conn+new Date().getTime()+'at'+Math.random(),type:'post',dataType:'json',data:$(obj_form).serialize(),timeout:opts.timeout,beforeSend:function(){process.output('pre');},error:function(result){process.output('err','',result.status+' '+result.statusText);setTimeout(function(){process.output();},opts.modal.delay);},success:function(result){if(typeof opts.jump.attach_key!='undefined'&&typeof result[opts.jump.attach_key]!='undefined'){var result_attach_value=result[opts.jump.attach_key];}
if(typeof result.msg=='undefined'&&result.err_message!='undefined'){result.msg=result.err_message;}
process.output('success',result.rcode,result.msg,result_attach_value);setTimeout(function(){process.output();},opts.modal.delay);}});}};var el={formSubmit:function(url){if(typeof url!='undefined'){opts.ajax_url=url;}
process.submitFunc();},ajaxUrl:function(url){if(typeof url!='undefined'){opts.ajax_url=url;}}};return el;};})(jQuery);